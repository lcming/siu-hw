...
Register_File_2R1W u_RF( .clk(clk), .addr_r1(inst[16:13]), .addr_r2(inst[12:9]), .wen_w(inst[21]), .addr_w(inst[20:17]), .data_w(data_write), .rf_data_r1(RF_read_1), .rf_data_r2(RF_read_2), .DM_addr_w1(inst[33:30]), .DM_addr_w2(inst[29:26]), .DM_data(data_in), .DM_wen_1(inst[35]), .DM_wen_2(inst[34]), .DM_addr_r(inst[25:22]), .acc(acc) );       
...
DW02_mult #(16, 16) u_mult( .A(M_in_1), .B(M_in_2), .TC(inst[37]), .PRODUCT(M_out) );
DW01_addsub #(16) u_addsub( .A(A_in_1), .B(A_in_2), .CI(1'b0), .ADD_SUB(inst[36]), .SUM(A_out), .CO(co) );
