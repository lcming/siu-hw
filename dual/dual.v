...
module dual(clk, inst1, inst2, in1, in2, M_sel, A_sel, S_sel, overflow);
...
wire [`DATA_W-1:0] A_out, M_out, S_out, A_in1, A_in2, M_in1, M_in2, S_in, q1_in, q1_out, s1_data, q2_in, q2_out, s2_data;
wire [1:0] A_in1_sel, M_in1_sel, S_in_sel, s1_op, s2_op;
wire [2:0] A_in2_sel, M_in2_sel;
wire q1_push, q1_pop, q2_push, q2_pop, A_cand, M_cand, S_cand;


// FU instantiation
DW02_mult #(`DATA_W, `DATA_W) u_mult ( .A(M_in1), .B(M_in2), .TC(tie_h), PRODUCT(M_out));
DW01_addsub #(`DATA_W, `DATA_W) u_addsub ( .A(A_in1), .B(A_in2), .CI(tie_l), .ADD_SUB(add_sub), .SUM(A_out));
DW_rash #(`DATA_W, `LOG_DATA_W) u_rash ( .A(S_in), .DATA_TC(tie_h), .SH(S_m), .SH_TC(tie_h), .B(S_out));

// RF instantiation
//module queue(clk, data_in, data_out, push, pop);
queue q1 (.clk(clk), .data_in(q1_in), .data_out(q1_out), .push(q1_push), .pop(q1_pop));
queue q2 (.clk(clk), .data_in(q2_in), .data_out(q2_out), .push(q2_push), .pop(q2_pop));
...
// wire connection //
// stack op
assign s1_op = inst1[3:4];
assign s2_op = inst2[3:4];

// wb 
assign q1_push = inst1[5];
assign q2_push = inst2[5];
...
always@(*)
begin: S_IN
    case({S_sel[1], inst1[0], inst2[0]})
        3'b00x: begin S_in = q1_out;  end
        3'b01x: begin S_in = s1_data; end
        3'b1x0: begin S_in = q2_out;  end
        3'b1x1: begin S_in = s2_data; end
        default: begin S_in = q1_out; end
    endcase
end

always@(*)
begin: A_IN2
    case( {A_sel[1], inst1[1:2], inst2[1:2]} )
...
always@(*)
begin: A_IN2
    case( {M_sel[1], inst1[1:2], inst2[1:2]} )
...
always@(*)
begin: Q1_IN
    case( {A_sel[0]&~A_sel[1], M_sel[0]&~M_sel[1], S_sel[0]&~S_sel[1]} )
        3'b1xx: begin q1_in = A_out; end
        3'bx1x: begin q1_in = M_out; end
        3'bxx1: begin q1_in = S_out; end
...
begin: Q2_IN
    case( {A_sel[0]&A_sel[1], M_sel[0]&M_sel[1], S_sel[0]&S_sel[1]} )
        ...
        3'bx1x: begin q2_in = M_out; end
        3'bxx1: begin q2_in = S_out; end
